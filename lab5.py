...
visited = [False for i in range(vert_num+1)]
def dfs(node, component):
    """Function that searches for graph vertices using the DFS method"""
    visited[node] = True
...
if __name__ == "__main__":
    ...
    #loop searching for connected vertices
    all_components = []
    for i in range(vert_num):
        if visited[i] == False:
            component = []
    ...
    print("\nAll connected vertices of the graph (standalone vertices have no connections):\n",list(all_components))

    print("\nAll connscted vertices (in pairs):\n",list(nx.edge_dfs(G)))
    ...
    #according to the documentation, when weight = None, the BFS method is used
    path=nx.shortest_path(G, source=source, weight = None,target=target) 
